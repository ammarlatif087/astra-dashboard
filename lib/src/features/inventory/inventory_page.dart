...
import 'package:astra_dashboard/src/features/inventory/data/product_model.dart';
import 'package:astra_dashboard/src/shared/widgets/layout/custom_table.dart';
    ...
    return productListManager.when(
      ready: (products) => MyCustomDataTable<DummyProduct>(
        ...
        onEdit: (data) async {
          showDialog(
              context: context,
              builder: (c) => AddEditProductDialog(product: data));
        },
        onDelete: (data) {
        ...
        createButton: ElevatedButton.icon(
          onPressed: () {
            showDialog(
                context: context, builder: (c) => const AddEditProductDialog());
          },
